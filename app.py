...
# 2. THE "NUCLEAR" CSS (Kills Vertical Line Error & Forces 4-Across)
st.markdown("""
    <style>
    /* Force Hardware Acceleration and kill the "White Screen" */
    html, body, [data-testid="stAppViewContainer"] { background: #fbfbfd !important; }
    
    /* THE GRID FIX: Force all widgets following the display to be 23% width and inline */
    [data-testid="stVerticalBlock"] > div:nth-child(n+4) {
    ...
    .stButton>button {
        width: 100% !important; border-radius: 12px !important; height: 60px !important;
...
with st.sidebar:
    ...
    with st.expander("Texas Data Privacy (TDPSA)"):
        ...
        st.caption("Compliant with Texas Data Privacy Act. No data transmitted.")
...
# 6. THE INLINE KEYPAD (No st.columns to prevent stacking!)
# We list them sequentially. CSS turns them into a 4-button row.
btns = [
    "AC", "√", "x²", "÷",
    "7", "8", "9", "×",
    "4", "5", "6", "-",
...
for label in btns:
    if st.button(label, key=f"btn_{label}", type="primary" if label == "=" else "secondary"):
