...
try:
    ...
    # REPAIR: We use 'gemini-1.5-flash' specifically. 
    # We removed '-latest' because your error showed the 'v1beta' API 
    # couldn't find that specific label.
...
if mode == "AI Symbolic Solver":
        ...
        if user_query:
            with st.spinner("🧠 Connecting to Google Neural Engine..."):
                try:
                    ...
                    # Using .text handles the response formatting automatically
                    st.write(response.text)
                except Exception as e:
                    # If it fails again, we display a clear diagnostic for the user
                    st.error(f"Google Server Error: {e}")
                    if "404" in str(e):
                        st.info("⚠️ **Diagnostic:** The API cannot find this model. Make sure your API Key was created using 'Create API key in NEW project' in Google AI Studio.")
        else:
    ...
    try:
        ...
        # Execute formula with NumPy context
...
st.sidebar.markdown("---")
st.sidebar.caption("Verilogic Pro v2.2")
