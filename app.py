...
# --- 3. THE NASA SYSTEM PREFERENCES (SIDEBAR) ---
with st.sidebar:
    st.title("Verilogic Pro")
    st.caption("v7.0.0 | System Preferences")
    st.write("---")
    ...
    with st.expander("📊 Variable Registry", expanded=True):
        if st.session_state.registry:
            ...
            st.caption("No custom variables saved.")

    with st.expander("📈 Graphing & Visualization"):
        st.toggle("Interactive 3D Plots", value=True)
        st.slider("Resolution", 50, 500, 200)

    with st.expander("⚙️ Computation & Output"):
        precision = st.slider("Math Precision", 5, 50, 10)
        if st.button("🖨️ Print / Save to PDF"):
    ...
    with st.expander("⚖️ Legal & About"):
        ...
        st.caption("Legal Statement: Verilogic Pro is an SRE tool. Developer holds no liability.")

# --- 4. THE ANALYTICAL ENGINE (INTENT BASED) ---
def ai_brain(query):
    q_raw = query.lower().strip()
    
    # NASA Constant Map (AstroPy)
    semantic_map = {
    ...
    if score > 85:
        ...
        # Save to Registry for Sidebar reference
        reg_key = best_match.replace(" ", "_")
        st.session_state.registry[reg_key] = f"{val:g} {unit}"
        
        try: wiki_summary = wikipedia.summary(best_match, sentences=3)
        except: wiki_summary = f"Verified scientific data for {best_match.title()} retrieved via NASA standards."
    ...
    if any(c.isdigit() for c in m_prep):
        try:
            ...
            return {
                ...
                "i": "Logic Core reduction verified via symbolic analysis.", 
    ...
    try:
        ...
        return {"q": query, "a": "Refining Inquiry", "i": "Inquiry unrecognized. Ensure scientific nouns or operators are used.", "t": "SYSTEM", "lx": None}
...
with st.form("main_interface", clear_on_submit=True):
    # Empty placeholder kills the "Ask Librarian" ghost text
