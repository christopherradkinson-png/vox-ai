...
# --- 2. THE DIRECT ENGINE (REPAIRED) ---
try:
    api_key = st.secrets["GOOGLE_API_KEY"]
    genai.configure(api_key=api_key)
    
    # REPAIR: Switched to 'gemini-1.5-flash-latest' to bypass 404 versioning errors
    ...
    if st.button("Execute Pro Logic"):
        if user_query:
            with st.spinner("🧠 Connecting to Google Neural Engine..."):
                try:
                    # Optimized prompt for cleaner output
                    response = model.generate_content(user_query)
                    st.markdown("### Engine Output")
                    st.write(response.text)
                except Exception as e:
                    st.error(f"Google Server Error: {e}")
                    st.info("If 404 persists, verify your API Key was created in 'A New Project' in AI Studio.")
        else:
...
st.sidebar.markdown("---")
st.sidebar.caption("Verilogic Pro v2.1")
