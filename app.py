...
st.markdown('<style>.stApp {background: #000;} h1 {text-align: center; color: #00d2ff !important; font-family: monospace; text-shadow: 0 0 30px #00d2ff, 0 0 50px #00d2ff; font-weight: 900;} h3 {text-align: center; color: #00d2ff !important; opacity: 1 !important;} .stButton>button {display: block; margin: 0 auto; width: 85%; background: rgba(0,210,255,0.2); border: 2px solid #00d2ff !important; color: #00d2ff !important; font-weight: bold; box-shadow: 0 0 15px #00d2ff;} .stTextInput input {background: #111 !important; color: #00d2ff !important; border: 2px solid #00d2ff !important; text-align: center; font-size: 1.5rem !important; font-weight: bold;} ::placeholder {color: #00d2ff !important; opacity: 1 !important;} [data-testid="stSidebar"] {background-color: #000 !important; border-right: 1px solid #ffaa00 !important;}</style>', unsafe_allow_html=True)

st.markdown('<h1>VERILOGIC-125 SRE</h1>', unsafe_allow_html=True)
st.markdown('<h3>// VOX_AI: FULL_AUTONOMY_ACTIVE //</h3>', unsafe_allow_html=True)
...
if calc and query:
    ...
    with st.status("VOX_AI_REASONING", expanded=True) as status:
        ...
        proc = query.lower()
        for w, v in W2N.items():
            proc = proc.replace(w, str(v))
        for w, s in OPS.items():
            proc = proc.replace(w, s)
...
st.sidebar.code('BUILD: 1.33.2-STABLE\nOS: SRE_PROD\nNODE: TEXAS')
