...
try:
    # Use the key from your Streamlit Secrets Vault
    api_key = st.secrets["GOOGLE_API_KEY"]
    genai.configure(api_key=api_key)
    
    # FINAL REPAIR: Switching to 'gemini-pro' (the most stable version)
    # This bypasses the 'v1beta' 404 error shown in your screenshot
        ...
        if query:
            with st.spinner("🧠 Connecting to Brain..."):
                try:
                    ...
                    st.info("If this persists, verify your API Key is active in Google AI Studio.")
        else:
