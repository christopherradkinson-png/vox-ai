...
def call_verilogic_engine(user_input):
    ...
    # PAYLOAD: Built piece-by-piece for absolute safety
    ...
    try:
        ...
        # If Google sends an error (like 400 or 403), we catch it here
        if response.status_code != 200:
            return f"API ERROR {response.status_code}: {response.text}"
            
        response_data = response.json()
        
        # NAVIGATION FIX: We must use [0] to get the first candidate and first part
        try:
            answer = response_data['candidates'][0]['content']['parts'][0]
            return answer
        except (KeyError, IndexError):
            return "Error: The engine returned data but the answer was not in the expected format."
...
st.caption("v18.0 | Stable Logic Bridge")
